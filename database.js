...
const connectionString = process.env.DATABASE_URL || process.env.PG_CONNECTION_STRING || 'postgresql://postgres:postgres@localhost:5432/demlicoin';
...
function querySync(sql, params = []) {
  ...
  client.query(statement, params, (err, res) => {
    error = err;
    result = res;
    done = true;
  });
...
function prepare(sql) {
  const preparedSql = sql.trim().replace(/select last_insert_rowid\(\) as id/ig, 'SELECT LASTVAL() AS id');
  return {
    run: (...params) => querySync(preparedSql, params),
    get: (...params) => {
      const res = querySync(preparedSql, params);
      return res.rows[0];
    },
    all: (...params) => {
      const res = querySync(preparedSql, params);
...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS site_ayarlari (
    ...
    kullanim_kosullari TEXT DEFAULT NULL
  );
